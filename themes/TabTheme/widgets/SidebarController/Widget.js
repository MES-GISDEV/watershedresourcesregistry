...
define([
  ...
  function(declare, lang, array, html, topic, aspect, query, on, mouse, fx, domGeometry,
	...
	//FK add var for desired length of tabs in sidebar controller
	var tabnum = 5;
    var clazz = declare([BaseWidget, PoolControllerMixin], {
      ...
      postMixInProperties: function() {
        ...
        this.nls.report = this.nls.report || "report";
        this.nls.reportPanels = this.nls.reportPanels || "Report Panels";
      },
      ...
      createTabs: function() {
        var allIconConfigs = this.getAllConfigs(),
		  ...
		  //FK replaced 4 with tabnum in if/else statement
        if (allIconConfigs.length <= tabnum + 1) {
          iconConfigs = allIconConfigs;
          this.moreTab = false;
        } else {
          iconConfigs = allIconConfigs.splice(0, tabnum);
          this.moreTab = true;
        }
        ...
        if (this.moreTab) {
          this.createTab({
            label: this.nls.report, //FK manually substituted this, ideally will create own tab for reports, was this.mls.more
            flag: 'more', //FK, if we add a new tab we need to change this up
      ...
      createTab: function(g) {
		...
		//FK Test for tab-not-selected tabs
            ...
            this.own(on(panel.domNode, 'keydown', lang.hitch(this, function(evt){
              if(evt.keyCode === keys.ESCAPE){
                ...
                if(iconNode && tab.flag !== "more"){
                  ...
                  var focusId = g.id;
				  //FK replaced 4 with tabnum
                  this.showMoreTabContent(this.tabs[tabnum], focusId);
                }
              }
            })));
          }));
      ...
      _createOtherGroupPaneTitle: function(focusIdx) {
        ...
        html.create('div', {
          'class': 'text',
          innerHTML: this.nls.reportPanels /*FK manually changed this from morePanels, 
           * if we create a report tab will need to adjust bac */
        }, node);
