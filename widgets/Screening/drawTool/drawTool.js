...
define([
  ...
  return declare([BaseWidget, _WidgetsInTemplateMixin, Evented], {
    ...
    postCreate: function () {
      ...
      this.selectAllLayers.click();
      //Turns off the selectable layer list when the widget first opens
      this.parentDivOfChooseSymbol.style.display = 'none';
    },
    ...
    _initLayers: function () {
      ...
      array.forEach(this._filteredLayerArray, lang.hitch(this, function (layerObject) {
        ...
        if (layerObject && layerObject.objectIdField && layerObject.geometryType) {
          ...
          }else if(this.layerItems.some(e => e.layerName == item.layerInfo.title) ){
            //makes sure there are no duplicates inside of the selectable layer list
            //GSC
            return;
          }else{
            item.init(layerObject);
    ...
    _getSelectableLayers: function () {
      var layers = [];
      //loop keeps going until it finds flo direction
      if((this.layerItems).some(e => e.layerName == 'FlowDirection'))
        var pop = "ok";
    ...
    _initializeDrawBoxTools: function () {
      var geometry, nepaButton;
      //GSC
      //finding the correct NEPA button and adding a number to it so it won't be selected again
      if(!document.getElementById("NEPA1")){
        nepaButton = document.getElementById("NEPA");
        nepaButton.id = "NEPA1";
        this.nepa = "NEPA1";
      }else if(!document.getElementById("NEPA2")){
        nepaButton = document.getElementById("NEPA");
        nepaButton.id = "NEPA2";
        this.nepa = "NEPA2";
      }else if(!document.getElementById("NEPA3")){
        nepaButton = document.getElementById("NEPA");
        nepaButton.id = "NEPA3";
        this.nepa = "NEPA3";
      }else if(!document.getElementById("NEPA4")){
        nepaButton = document.getElementById("NEPA");
        nepaButton.id = "NEPA4";
        this.nepa = "NEPA4";
      }
      //Finding the layer select section
      var layerSelect = this.parentDivOfChooseSymbol;
      ...
      this.own(on(this._drawTool, "draw-activate", lang.hitch(this, function (tool) {
        ...
        if (!this.selectTool.focused) {
          //adds NEPA button if select tool is on
          document.getElementById(this.nepa).style.display = 'block';
          //Turns off the selectable layer list when a draw tool is selected
    ...
    _initializeSelectTool: function () {
      ...
      this.own(on(selectButton, "click", lang.hitch(this, function () {
        ...
        if (this.selectTool.isActive()) {
          this.map.disablePan();
          //removes NEPA button if select tool is on
          document.getElementById(this.nepa).style.display = 'none';
          //Turns on the selectable layer list when the select tool is on
          layerSelect.style.display = 'block';
        }else{
          //adds NEPA button if select tool is off
          document.getElementById(this.nepa).style.display = 'block';
          //Turns off the selectable layer list when the select tool is off
          layerSelect.style.display = 'none';
        }
