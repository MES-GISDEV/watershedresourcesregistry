...
define([
	...
	return declare([_WidgetBase, _TemplatedMixin, BaseWidget], { //FK 2022-12-28, added BaseWidget
		...
		addLayerNode: function(layerInfo, level, toTableNode, position) {
			...
			//alert(String(layerInfo.id)); //FK Testing
			...
			try {
				if (!layerInfo.isVisible()) {
					//IMPORTANT! this _fromFC is being produced over at \widgets\AddWebMapData\widget.js'
					//This allows map feature collection layers to show up in the Active Layer List even
			...
			/*FK this was added so the display:none style can be added, rendering rows invisible
			as well as an ID to the layer 'storage' node, so we can easily make them visble later*/
			...
			layerTrNode = domConstruct.create('tr', {
				...
				'id': 'ID_' + layerInfo.id, //FK added so an element could be easily ID'd
				'style': (sdClass == 'not-visible' ? 'display:none;' : '')
			});
			...
			if (layer){
				...
				var chLayer = layer.id;
				var chMainElm = document.getElementById("ID_" + chLayer);
			...
			//FK -- the code immediately above replaces this
			//var layerTrNodeClass = "layer-tr-node-" + layerInfo.id;
