...
function(BaseWidget, declare, lang, array, html, dom, on,
	...
	var clazz = declare([BaseWidget], {
		...
		showLayers: function() {
			...
			//FK -- I don't think we need this but I am going to keep this here just in case
			/*myArray = []
				array.forEach(this.operLayerInfos.getLayerInfoArray(), function(layerInfo){
				if (layerInfo){
				myNum = layerInfo.isVisible()
				if (myNum == true) {
				myArray.push(layerInfo)
				}
				}
			}, this);*/
			...
			this.layerListView = new LayerListView({
				...
				config: this.config,
				map: this.map //FK - need to include the map from the widget so LayerListView can see layers
