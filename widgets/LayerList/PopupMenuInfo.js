	...
	var clazz = declare([], {
		...
		_initCandidateMenuItems: function() {
			...
			this._candidateMenuItems = [{
				...
				key: 'remove', //FK Testing 2022-12-16
				label: this.nls.itemRemove //FK Testing 2022-12-16
				},{
				key: 'moveup',
		...
		*/
		...
		onPopupMenuClick: function(evt) {
			...
			switch (evt.itemKey) {
				...
				break;
				case 'remove': //FK Testing 2022-12-16
				this._onItemRemoveClick();
				break; //FK Testing 2022-12-16
		...
		**********************************/
		...
		_onItemRemoveClick: function(evt) {
			...
			if(lyr){  
				lyr.setVisibility(false) //FK Testing 2022-12-16 I am purposely turning off the layer
				//because WRR Layer List is synched up with this list
				
				/*Testing 2022-12-19 Can we get open the right file below based on web address or
				whatever state we have up? */
				...
				/*Testing 2022-12-19 Attempt is to talk to WRR Layer List for respective state
				see if the ID exists in there, if so break for now (there's probably more to do
				this may even be moot, but baby steps*/
				var wrrRequest = new XMLHttpRequest();
				//wrrRequest.open('GET', './appInfo.json', false); testing just to read something
				wrrRequest.open('GET', myPath, false)
				wrrRequest.send(null)
				var myJSONObject = JSON.parse(wrrRequest.responseText);
				//alert(myJSONObject.name); testing to see something
				/*Create a loop with the layers, if the layerId matches a name in the list, stop
				for now */
				var removeMyItem = true;
				...
				/*If we didn't find an item up, go ahead and remove*/
				if (removeMyItem) {
					this._layerInfo.map.removeLayer(lyr);
				}
				else { //hold up for now, give an alert for now
					alert ('WRR Layer, holding up on removing for now');
				}
