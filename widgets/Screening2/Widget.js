...
define([
  ...
  return declare([BaseWidget, _WidgetsInTemplateMixin], {
    ...
    onOpen: function () {
      ...
      }else if (this.map._layers.drawnOrSelectedGraphicsLayer && this.map._layers.drawnOrSelectedGraphicsLayer.graphics) {
        this._drawnOrSelectedGraphicsLayer = this.map._layers.drawnOrSelectedGraphicsLayer;
        this._drawnOrSelectedGraphicsLayer.show();
      }
      if (this._aoiGraphicsLayer && this._aoiGraphicsLayer.graphics) {
        this._aoiGraphicsLayer.show();
      }else if (this.map._layers.aoiGraphicsLayer && this.map._layers.aoiGraphicsLayer.graphics) {
        this._aoiGraphicsLayer = this.map._layers.aoiGraphicsLayer;
        this._aoiGraphicsLayer.show();
      }
     ...
     */
    onClose: function () {
      //don't remove aoi graphics from the map when closing widget
    ...
    _attachAllEvents: function () {
      ...
      //Creating the NEPA Assist button
      var nepaButton = document.getElementById("NEPA");
      this.own(on(nepaButton, "click", lang.hitch(this, this._NEPAAssistLink)));
    },
    //function for the NEPA Assist link inside of location details
    _NEPAAssistLink: function (evt) {
      var outputX;
      var outputY;
      var shape = this.graphics[0].geometry.type;
      var buffer = this.bufferDistanceTextBox.value;
      var bufferValue = this.bufferDistanceTextBox.value;
      var bufferUnit = this.bufferDistanceUnit.value;
      var nepaButton = document.getElementById("NEPA");
      var params = new ProjectParameters();
      var outSR = new SpatialReference({
        wkid: 4326 //Web Mercator Projection
      });

      if(bufferUnit == "UNIT_STATUTE_MILE"){
        bufferUnit = "miles";
      }else if(bufferUnit == "UNIT_KILOMETERS"){
        bufferUnit = "kilometers";
      }else if(bufferUnit == "UNIT_FOOT"){
        bufferUnit = "miles";
        buffer = buffer/5280;
      }else if(bufferUnit == "UNIT_METERS"){
        bufferUnit = "kilometers";
        buffer = buffer/1000;
      }

      if(shape == "point"){
        var inputPoint = new esri.geometry.Point(this.graphics[0].geometry.x, this.graphics[0].geometry.y, this.graphics[0].geometry.spatialReference);
        params.geometries = [inputPoint];
        params.outSR = outSR;

        this._geometryService.project(params, function(outputArr){
          outputX = outputArr[0].x;
          outputY = outputArr[0].y;

          var aoiCoordinates = (outputX.toPrecision(8)).toString()  +","+ (outputY.toPrecision(8)).toString();
          //need to open blank window first for the map to show up on NEPA Assist Report
          var nepaWindow = window.open("", "_blank");
          if(bufferValue == 0 || bufferValue == 1){
            nepaWindow.open("https://nepassisttool.epa.gov/nepassist/analysis.aspx?ptitle=WRR Analysis&coords="+ aoiCoordinates + "&type=point&radius=1&unit=" + bufferUnit + "&f=report", "_self");
          }else{
            nepaWindow.open("https://nepassisttool.epa.gov/nepassist/analysis.aspx?ptitle=WRR Analysis&coords="+ aoiCoordinates + "&type=point&radius=" + buffer + "&unit=" + bufferUnit + "&f=report", "_self");            
          }
        });
      }else if(shape == "polyline"){
        var inputLine = new esri.geometry.Polyline(this.graphics[0].geometry);
        params.geometries = [inputLine];
        params.outSR = outSR;

        this._geometryService.project(params, function(outputArr){
          var aoiCoordinates = "";
          for (var i = 0; i < outputArr[0].paths[0].length; i++) {
            outputX = outputArr[0].paths[0][i][0];
            outputY = outputArr[0].paths[0][i][1];
            if(i == outputArr[0].paths[0].length - 1){
              aoiCoordinates = aoiCoordinates + (outputX.toPrecision(8)).toString()  +","+ (outputY.toPrecision(8)).toString();
            }else{
              aoiCoordinates = aoiCoordinates + (outputX.toPrecision(8)).toString()  +","+ (outputY.toPrecision(8)).toString() + ",";
            }
          }
          ...
          var nepaWindow = window.open("", "_blank");
          if(bufferValue == 0 || bufferValue == 1){
            nepaWindow.open("https://nepassisttool.epa.gov/nepassist/analysis.aspx?ptitle=WRR Analysis&coords="+ aoiCoordinates + "&type=line&radius=1&unit=" + bufferUnit + "&f=report", "_self");
          }else{
            nepaWindow.open("https://nepassisttool.epa.gov/nepassist/analysis.aspx?ptitle=WRR Analysis&coords="+ aoiCoordinates + "&type=line&radius=" + buffer + "&unit=" + bufferUnit + "&f=report", "_self");            
          }
        });
      }else if(shape == "polygon"){
        var inputPolygon = new esri.geometry.Polygon(this.graphics[0].geometry);
        params.geometries = [inputPolygon];
        params.outSR = outSR;

        this._geometryService.project(params, function(outputArr){
          var aoiCoordinates = "";
          for (var i = 0; i < outputArr[0].rings[0].length; i++) {
            outputX = outputArr[0].rings[0][i][0];
            outputY = outputArr[0].rings[0][i][1];
            if(i == outputArr[0].rings[0].length - 1){
              aoiCoordinates = aoiCoordinates + (outputX.toPrecision(8)).toString()  +","+ (outputY.toPrecision(8)).toString();
            }else{
              aoiCoordinates = aoiCoordinates + (outputX.toPrecision(8)).toString()  +","+ (outputY.toPrecision(8)).toString() + ",";
            }
          ...
          var nepaWindow = window.open("", "_blank");
          //need to open blank window first for the map to show up on NEPA Assist Report
          //var nepaWindow = window.open("https://nepassisttool.epa.gov/nepassist/analysis.aspx?ptitle=WRR Analysis&coords="+ aoiCoordinates + "&type=line&radius=" + buffer + "&unit=" + bufferUnit + "&f=report", "_self");
          //setTimeout(function(){window.open("https://nepassisttool.epa.gov/nepassist/analysis.aspx?ptitle=WRR Analysis&coords="+ aoiCoordinates + "&type=polygon&radius=&unit=" + bufferUnit + "&f=report", "_parent");}, 3000);
          nepaWindow.open("https://nepassisttool.epa.gov/nepassist/analysis.aspx?ptitle=WRR Analysis&coords="+ aoiCoordinates + "&type=line&radius=" + buffer + "&unit=" + bufferUnit + "&f=report", "_self");  
      ...
      switch (geometryType) {
        ...
        case "esriGeometryPolyline":
          if (domClass.contains(this.standardUnitAreaContainer, "esriCTHidden")) {
            colTitle = "Clipped " + this.nls.reportsTab.featureLengthText +
        ...
        case "esriGeometryPolygon":
          if (domClass.contains(this.standardUnitAreaContainer, "esriCTHidden")) {
            colTitle = "Clipped " + this.nls.reportsTab.featureAreaText +
    ...
    _getPrintTemplate: function () {
      ...
      printTemplate.layoutOptions = {
		...
		//FK added the titleText to this, this is a layout option. I spent a half a day only
		//to see that if I Google "printTemplate javascript API", I found this
		titleText: this.printLayoutTitle.value
      };
    ...
    _getProcessedPrintData: function () {
      var dataForReport, areaOfInterestText, areaOfInterestValue, aoiTextTemplate,
        ...
        summaryTableMeasurement, aoiCoordinates;
      dataForReport = [];
      ...
      aoiCoordinates = (((this.map.geographicExtent.ymax+this.map.geographicExtent.ymin)/2).toPrecision(6)).toString() +", "+ (((this.map.geographicExtent.xmin+this.map.geographicExtent.xmax)/2).toPrecision(6)).toString();
      ...
      aoiTextTemplate =
        ...
        "<div>Lat/Long: " + aoiCoordinates + "</div></div>";
      aoiText = {
        title: "",
        type: "html",
      ...
      if (this.config.reportSettings && this.config.reportSettings.printTaskURL) {
        if (this.config.reportSettings.logo) {
          if (this.config.reportSettings.logo.indexOf("${appPath}") > -1) {
            //changed the url to correctly return the logo path
    ...
    _createNewGraphicsLayer: function (layerId) {
      ...
      //don't remove aoi graphics that are already on the map
      if(layerId == "toleranceGraphicsLayer" || layerId == "uploadedShapefileGraphicsLayer" || 
        !this.map._layers["aoiGraphicsLayer"] || !this.map._layers["drawnOrSelectedGraphicsLayer"]){
        if (layerId) {
        ...
        newGraphicsLayer = this.map._layers[layerId];
        return newGraphicsLayer;
      }
    },
    ...
    _initializeDrawToolsWidget: function () {
      ...
      this.own(on(this._drawTool, "onDrawComplete", lang.hitch(this, function (graphics) {
        ...
        //creates a graphics variable that is used to find the AOI location
    ...
    _initToCreateAOIBuffer: function (graphics) {
      ...
      if (graphics[0].geometry.type == "polygon") {
          console.log('polygon');
          var areaAcres = GeometryEngine.geodesicArea(graphics[0].geometry, "acres");
          console.log(areaAcres);
          if (areaAcres > 100) {
              this.errMessageAreaTooBig.innerHTML = "**Area selected is above 100 acres, report may take some time to generate.**"
          } else {
              this.errMessageAreaTooBig.innerHTML = "";
          }
      } else {
          this.errMessageAreaTooBig.innerHTML = "";
      }	  
    ...
    _disableShowReportsButton: function () {
      ...
      domClass.add(this.nepaButton, "jimu-state-disabled");
    },
    ...
    _enableShowReportsButton: function () {
      ...
      domClass.remove(this.nepaButton, "jimu-state-disabled");
    },
    ...
    _setUpPrintDialogPopup: function () {
      ...
      this.own(on(this.printActionButton, "click", lang.hitch(this, function () {
        ...
        if (this.reportDijit) {
		  ...
		  //FK changed the application, so user can adjust title on popup screen 2022-08-19
          //this.reportDijit.print(this.config.reportSettings.reportTitle, data); //old code
		  this.reportDijit.print(this.printLayoutTitle.value, data);
        } else {
          this._showMessage(this.nls.reportsTab.errorInPrintingReport);
        }
