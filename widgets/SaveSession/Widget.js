    ...
    function (declare,
        ...
        return declare([BaseWidget, _WidgetsInTemplateMixin], {
            ...
            initSavedSessionUI: function () {
                ...
                //this.own(on(this.sessionTable, 'actions-download', lang.hitch(this, 'onSaveToFileButtonClicked')));
            ...
            onSaveButtonClicked: function () {
                ...
                if (!this.isValidSessionName(sessionName)) {
                    console.log('SaveSession :: onSaveButtonClicked :: invalid session name = ', sessionName);
                    return;
                }

                // if(this.totalSessions.length >= 8){
                //   alert("Max number of total sessions reached (8) on all WRR maps, please delete some to make room for other sessions.");
                //   return;
                // }
                ...
                this.totalSessions.push(session);
            ...
            onSessionTableChanged: function (e) {
                ...
                //create new array that holds updated total sessions after removing an item from current map
                this.totalSessions2 = this.sessions;
                for (var i = 0; i < this.totalSessions.length; i++) {
                    if(this.map.itemId != this.totalSessions[i].webmapId){
                        this.totalSessions2.push(this.totalSessions[i]);
                    }
                }
                this.totalSessions = this.totalSessions2;
                this.storeSessions();
            ...
            onSaveToFileButtonClicked: function (e) {
                ...
                //var sessionString = JSON.stringify(this.totalSessions);
            ...
            onSaveItemToFileClicked: function (e) {
                ...
                // if (!this.config.useServerToDownloadFile) {
                //     // skipping since there is no url to submit to
                //     console.log('SaveSession :: onSaveItemToFileButtonClicked :: saveItemToFileForm submit canceled.');
                //     return;
                // }
                ...
                // trigger the post to server side
                //this.saveToFileForm.namespaceURI = "";
                this.saveToFileForm.submit();
            ...
            loadSavedSessionsFromFile: function (file) {
                ...
                reader.onload = function () {
                    ...
                    array.forEach(sessionsToLoad, function (sessionToLoad) {
                        ...
                        this.totalSessions.push(sessionToLoad);
            ...
            storeSessions: function () {
                ...
                var stringToStore = JSON.stringify(this.totalSessions);
                try{
                    localStorage.setItem(this.storageKey, stringToStore);
                    console.log("SaveSession :: storeSessions :: completed");
                }catch(err){
                    // alert(err.message +":\nToo much memory has been used. Please delete some sessions to make room." 
                    //     + "\nThis could also include sessions saved in other WRR maps.");
                    alert(err.message +":\nThe session will not be saved to local storage but will still appear in the list below." 
                        + "\nClick the 'Save to file' link to save your most recent session for future use or it will be lost."
                        + "\nTry deleting some sessions to make room in local storage."
                        + "\nThis could also include sessions saved in other WRR maps.");
                    console.log("SaveSession :: storeSessions :: failed");
                    //throw "Too much memory used.";
                }
            ...
            loadSavedSessionsFromStorage: function () {
                ...
                // replace to current sessions using only sessions that match the current map
                this.sessions = [];

                for (var i = 0; i < storedSessions.length; i++) {
                     if(this.map.itemId == storedSessions[i].webmapId){
                            this.sessions.push(storedSessions[i]);
                     }
                 }

                // replace to total sessions
                //this.sessions = storedSessions;
                this.totalSessions = storedSessions;
